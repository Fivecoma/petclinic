    ...
    stages {
        ...
        stage('Deploy to K8s') {
            steps {
                ...
                sh 'ssh root@192.168.60.60 kubectl apply -f /home/vagrant/deployment.yml'
                sh 'ssh root@192.168.60.60 kubectl delete svc my-service'
                sh 'ssh root@192.168.60.60 kubectl expose deployment petclinic-deployment --type=NodePort --port=8080 --name=my-service'
            }
        }
    }
}
